		...
		describe "#post" do
			context "when params are valid" do
				it "respond with movement" do
					...
					post "/api/v1/movements", {"access_token" => auth[:token], "movement" => movement}
	...
	describe "/:id" do
		describe "#get" do 
			...
			context "when open movement is accessed under ban" do
				it "respond with 403" do
	  			...
	  			@movement.memberships << Membership.new({movement: @movement, user: auth[:user], role: "banned"})
					@movement.save!
					get "/api/v1/movements/#{@movement.id}.json", {"access_token" => auth[:token]}
		    	expect(response.status).to eq(403)
		    	expect(json["error"]).to eq("Unable to retrieve this movement")
			...
			context "when closed movement is found as a member" do
				it "respond with object" do
					@movement = FactoryGirl.create(:movement, privacy: "closed")
	  			@movement.memberships << Membership.new({movement: @movement, user: auth[:user], role: "member"})
					@movement.save!
					get "/api/v1/movements/#{@movement.id}.json", {"access_token" => auth[:token]}
		    	expect(response).to be_success
		    end
		...
		describe "#delete" do
			context "when movement cannot be found" do
				it "respond with an error" do
					delete "/api/v1/movements/1.json", {"access_token" => auth[:token]}
		    	expect(response.status).to eq(404)
			...
			context "when movement is deleted with incorrect permissions" do
				it "respond with an error" do
					movement = FactoryGirl.create(:movement)
					delete "/api/v1/movements/#{movement.id}.json", {"access_token" => auth[:token]}
		    	expect(response.status).to eq(403)
			...
			context "when one movement is deleted with correct permissions" do
				it "respond with success" do
					@movement = FactoryGirl.create(:movement)
					@movement.memberships << Membership.new({movement: @movement, user: auth[:user], role: "admin"})
					@movement.save!
