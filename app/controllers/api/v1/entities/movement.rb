    ...
    module Entities
      class Movement < Grape::Entity
        ...
        expose :name,
               documentation: { required: true,
                                type: 'string',
                                desc: 'Name of movement' }
        expose :short_description,
               documentation: { required: true,
                                type: 'integer',
                                desc: 'Brief (150 Char Max) description' }
        expose :followers,
               documentation: { required: true,
                                type: 'integer',
                                desc: 'Number of people following' } \
        do |movement, _options|
        ...
        expose :role,
               documentation: { required: true,
                                type: 'string',
                                desc: 'admin, moderator, author,
                                member, invited, banned, none' } \
        do |movement, options|
          role = Membership.where(user_id: options[:user].id,
                                  movement_id: movement.id).first.try(:role)
